				...
				<article>
					<table style="width: 100%;">
						<c:forEach items="${matches}" var="i">
							...
							<td>${i.id}</td>
							<td>${i.title}
<%-- TODO: Uncomment this to see issues with lazy initialization --%>
<%--
         <c:choose>
            <c:when test="${not empty i.inventoryRecords}">
               ( <c:forEach items="${i.inventoryRecords}" var="curInv">
               ${curInv.location}, ${curInv.quantity} | 
                 </c:forEach>
               )
            </c:when>
          </c:choose>
